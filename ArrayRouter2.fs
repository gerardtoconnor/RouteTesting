...
module Giraffe.HttpRouteArray2
...
let router2 (paths: PathNode list) =
    ...
    let processPath(path:string,ctx) =
        ...
        let rec parsing (p,n,retry,fp,prs:Parser,result:obj []) : Task<HttpContext option> =                
            ...
            // Parse function begin
                ...
                | Parsy ->
                    match fary.[int nary.[n].Hop] with
                    | ParseStart (i,prs,fp) ->
                        let nresult = Array.zeroCreate<obj>(i) 
                        parsing(p,n + 1,p,fp,prs,nresult)
                    | ParseMid (i,prs,nfp) ->
                        parsing(p,n + 1,p,nfp,prs,result)                        
                    | xfn -> failwith(sprintf "unhandled parse Continue function match case %A" xfn) 
                | Endy  ->
                    parseEnding(path,ctx,p,fary.[int nary.[n].Hop],noneTask)
                | Forky ->
                ...
                | x ->
                    let rec tryPath(ip) =
                        if ip < path.Length then 
                            if path.[ip] = x then
                                //possible match
                                parsing(ip + 1,int nary.[n].Hop,retry,fp,pe,prs,results)
                            else
                                tryPath (ip + 1)
                        else 
                            if fp = 0 then
...
let inline route2 (path:string) = PathNode(Route path)

// type Bindy() =
//     member x.EatMe<'U,'T> (sf:StringFormat<'U,'T>) (fn : 'T -> HttpHandler) (v2:obj) = v2 :?> 'T |> fn
// let bindy = Bindy()
let route2f (fmt:StringFormat<'U,'T>) (fn:'T -> HttpHandler) =
    let path = fmt.Value
